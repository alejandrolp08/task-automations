...
async function updateSmartsuiteListingFields(records, { platformLabel = "ReachPro" } = {}) {
  ...
  const platformField = (schema.structure || []).find((field) => field.slug === BUYING_SMARTSUITE.fields.platformListedOn) || null;
  ...
  for (const record of records) {
    ...
    if (!currentEventId && resolvedEventId) {
      payload[BUYING_SMARTSUITE.fields.eventId] = resolvedEventId;
    }
    ...
    if (
      ...
      payload[BUYING_SMARTSUITE.fields.platformListedOn] = [platformChoice.value];
    }
